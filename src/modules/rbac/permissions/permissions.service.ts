...
export class PermissionsService {
  ...
  /**
   * Récupérer tous les endpoints sécurisés par le décorateur @Permission,
   * regroupés par contrôleur.
   */
  async getEndpointsByController() {
    ...
    // Parcourir tous les modules de l'application
    for (const moduleRef of this.modulesContainer.values()) {
      // Parcourir tous les contrôleurs du module
      for (const wrapper of moduleRef.controllers.values()) {
        ...
        // Récupérer le chemin de base du contrôleur (défini par @Controller('path'))
        const controllerPath = Reflect.getMetadata('path', metatype) || '';
        const prototype = Object.getPrototypeOf(instance);

        // Récupérer les méthodes du contrôleur
        ...
        for (const methodName of methodNames) {
          ...
          // Vérifier si la méthode est décorée avec @Permission
          const permission = this.reflector.get<string>('permission', methodRef);
          if (!permission) {
            continue;
          }

          // Récupérer les métadonnées de routage de NestJS (chemin et méthode HTTP)
          const pathMetadata = Reflect.getMetadata('path', methodRef);
          const requestMethodMetadata = Reflect.getMetadata('method', methodRef);

          // Associer le code de la méthode HTTP à son nom (GET, POST, etc.)
          let httpMethod = 'UNKNOWN';
          if (requestMethodMetadata !== undefined) {
            httpMethod = RequestMethod[requestMethodMetadata] || 'UNKNOWN';
          }

          // Gérer le cas où un endpoint a plusieurs chemins définis sous forme de tableau
          const paths = Array.isArray(pathMetadata)
            ? pathMetadata
            : typeof pathMetadata === 'string'
              ? [pathMetadata]
              : [''];

          for (const path of paths) {
            const cleanControllerPath = typeof controllerPath === 'string' ? controllerPath : '';
            const cleanMethodPath = typeof path === 'string' ? path : '';
            let fullPath = `/${cleanControllerPath}/${cleanMethodPath}`.replace(/\/+/g, '/');
            if (fullPath.endsWith('/') && fullPath.length > 1) {
              fullPath = fullPath.slice(0, -1);
            }

            // Récupérer le summary Swagger de l'opération si disponible
            const apiOperation = Reflect.getMetadata('swagger/apiOperation', methodRef);
